...
![image-20231027143723573.png](./images/image-20231027143723573.png)
...
![452542](./images/452542)

填写备注内容，也可以不填直接Enter

![64024242](./images/64024242)

在STASHES菜单中可以看到保存的stash

![64045245](./images/64045245)

先点击stash记录旁的小箭头，再点击 apply 或者 pop 都可恢复 stash

![4527825785278](./images/4527825785278)
...
![640452452452452](./images/640452452452452)
...
![64045245245](./images/64045245245)

需要把 b 复制到另一个分支，首先把 commitHash 复制下来，然后切到 master 分支。

![640452452452](./images/640452452452)

当前 master 最新的记录是 a，使用`cherry-pick`把 b 应用到当前分支。

![640452540210](./images/640452540210)
...
![6403432645](./images/6403432645)

因为 revert 会生成一条新的提交记录，这时会让你编辑提交信息，编辑完后 :wq 保存退出就好了。

![64063436](./images/64063436)
...
![6403431](./images/6403431)

现在的 master 分支里多了条合并提交。

![6403654363](./images/6403654363)
...
![6403643643](./images/6403643643)

现在 master 的记录是这样的。

![6404364363](./images/6404364363)
...
![6406356353](./images/6406356353)

分支记录如上，想要 reset 到 b。

![6403656365](./images/6403656365)

误操作 reset 过头，b 没了，最新的只剩下 a。

![640536563653](./images/640536563653)

这时用`git reflog`查看历史记录，把错误提交的那次 commitHash 记下。

![640365353](./images/640365353)
