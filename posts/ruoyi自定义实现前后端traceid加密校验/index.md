...
![image-20240429101925819.png](./images/image-20240429101925819.png)
...
[来源](https://www.jianshu.com/p/860d1f59b7f9)原因是：DES 秘钥采用，每字节取前七位为有效位，第八位为奇偶校验位，所以只要每字节去前七比特位一样，秘钥就是等效的；       

就是说上面采用等效（或一样）的秘钥


DES（数据加密标准）作为一种对称加密算法，其特点是使用相同的密钥进行加密和解密。然而，您提到的情况——不同的密钥加密同一明文得到相同密文——在理论上是不可能发生的，因为DES算法设计上确保了密钥的任何微小变化都应导致加密结果的显著不同。这是基于DES算法的混淆和扩散原则，目的是为了实现加密过程的非线性及对密文的随机化，从而保证加密的安全性。

如果观察到不同密钥加密同一明文得到相同密文，可能的原因包括但不限于以下几点误解或错误配置：

1. 编码问题：加密前的数据如果因为字符编码不同（如UTF-8与其他编码），导致虽然看起来是相同的明文，但实际上二进制表示不同，这不会是因为密钥的问题，而是输入本身就不一致。
2. 加密模式或初始化向量（IV）问题：在某些加密模式下（如CBC、CFB等），相同的明文和密钥可能因为使用了相同的IV而产生相同的密文。但DES本身不直接使用IV，这更可能是使用了如DESede（3DES）或某种模式下的误配。
3. 软件实现错误：使用的加密库或自行编写的加密代码可能存在错误，没有正确地使用密钥，或者在密钥管理和密钥调度上出现了问题。
4. 特定的密钥对：极为罕见的情况下，可能存在数学上的异常或特定的密钥对，但这违背了DES算法的安全属性，且在实际应用中几乎不可能遇到。
